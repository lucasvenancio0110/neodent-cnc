...
function passaFiltro(item) {
  ...
  if (filtro === "MANUTENÇÃO") return s.includes("MANUT");
  if (filtro === "APOIO") return Number(item.precisa_apoio || 0) === 1;
  return s.includes(filtro);
}

function card(item) {
  const tnl = String(item.tnl || "").padStart(3, "0");
  const apoio = Number(item.precisa_apoio || 0) ? `Apoio: ${item.apoio_status || "AGUARDANDO"}` : "Sem apoio";
  return `
    <article class="machine-card ${statusClass(item.status)}">
        ...
        <div class="machine-main">
          ...
          <div class="machine-name">
            ...
            <div class="machine-mini">
              ...
              <span>${item.aberto_por || "Sistema"}</span>
            </div>
          </div>
        </div>
      ...
      <div class="machine-body">
        <div class="machine-detail"><b>${item.motivo || "Sem motivo"}</b><br>${item.detalhe || item.observacao || "Sem detalhe"}</div>
        <div class="machine-actions">
          <button class="btn btn-ok" data-action="concluir" data-id="${item.id}">Concluir</button>
          ${Number(item.precisa_apoio || 0) ? `<button class="btn btn-warn" data-action="assumir" data-id="${item.id}">Vou atender</button>` : ""}
        </div>
      </div>
    </article>
  `;
...
function render() {
  ...
  board.innerHTML = items.length ? items.map(card).join("") : `<div class="empty-state">Nenhum card para esse filtro.</div>`;
}

async function load() {
  board.innerHTML = `<div class="empty-state">Carregando painel...</div>`;
  try {
...
chips.forEach((chip) => {
  chip.addEventListener("click", () => {
    ...
    filtro = chip.textContent.trim().toUpperCase();
    render();
  });
});
