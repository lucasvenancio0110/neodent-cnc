...
function responsibleForm(){if(!activeModalItem?._key)return;const current=person(activeModalItem)==="Sem dono"?"":person(activeModalItem);const type=activeModalItem.responsavel_tipo||"Operador";aoModalMsg.innerHTML=`<form class="ao-inline-form" data-resp-form><label><span>Quem vai fazer?</span><input id="respNome" value="${safe(current)}" placeholder="Ex.: Juliano" autocomplete="off"></label><label><span>Tipo</span><select id="respTipo"><option ${type==="Operador"?"selected":""}>Operador</option><option ${type==="Preparador"?"selected":""}>Preparador</option><option ${type==="Técnico"?"selected":""}>Técnico</option><option ${type==="Líder"?"selected":""}>Líder</option></select></label><div class="ao-inline-row"><button type="submit" data-save-responsavel>Salvar</button><button type="button" data-cancel-inline>Cancelar</button></div></form>`;setTimeout(()=>document.getElementById("respNome")?.focus(),80);}
function saveResponsible(){const name=clean(document.getElementById("respNome")?.value||"");const type=clean(document.getElementById("respTipo")?.value||"Operador");if(!name){document.getElementById("respNome")?.focus();return;}doPatch({responsavel_nome:name,responsavel_tipo:type,responsavel_indicado_por_nome:userName(),responsavel_indicado_em:new Date().toISOString(),status_atividade:"responsavel_indicado",situacao:"RESPONSAVEL_INDICADO"},`${name} indicado como responsável.`);}
function observationForm(){if(!activeModalItem?._key)return;aoModalMsg.innerHTML=`<form class="ao-inline-form" data-obs-form><label><span>Observação</span><textarea id="obsTexto" placeholder="Ex.: Preset ainda não trouxe"></textarea></label><div class="ao-inline-row"><button type="submit" data-save-observacao>Salvar</button><button type="button" data-cancel-inline>Cancelar</button></div></form>`;setTimeout(()=>document.getElementById("obsTexto")?.focus(),80);}
function saveObservation(){const obs=clean(document.getElementById("obsTexto")?.value||"");if(!obs){document.getElementById("obsTexto")?.focus();return;}const base=clean(activeModalItem.observacao||"");doPatch({observacao:base?`${base}\n${userName()}: ${obs}`:`${userName()}: ${obs}`,observado_por_nome:userName(),observado_em:new Date().toISOString()},"Observação adicionada.");}
...
aoCardModal.addEventListener("submit",e=>{if(e.target.matches("[data-resp-form]")){e.preventDefault();saveResponsible();}if(e.target.matches("[data-obs-form]")){e.preventDefault();saveObservation();}});
aoCardModal.addEventListener("click",e=>{if(e.target.closest("[data-close-ao-modal]"))closeCardModal();if(e.target.closest("[data-cancel-inline]")){aoModalMsg.textContent="Escolha uma ação.";return;}const btn=e.target.closest("[data-future-action]");if(!btn)return;const action=btn.dataset.futureAction;if(action==="assumir")assumeActiveItem();else if(action==="responsavel")responsibleForm();else if(action==="iniciar")startActiveItem();else if(action==="concluir")finishActiveItem();else if(action==="observacao")observationForm();else if(action==="apoio")requestSupport();});
