const VERSION = "worker-v4-auth-neodent-cnc";
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path === "/status") {
        return ok({ ok: true, version: VERSION, now: new Date().toISOString() });
      }

      if (path === "/auth/bootstrap-admin" && request.method === "POST") {
...
async function bootstrapAdmin(request, env) {
  ...
  if (Number(total?.total || 0) > 0) return fail("Bootstrap bloqueado: ja existe ADMIN", 403);

  const nome = clean(body.nome || "Administrador");
  const login = clean(body.login || "admin").toLowerCase();
  const senha = String(body.senha || "");
  if (senha.length < 6) return fail("Senha inicial precisa ter pelo menos 6 caracteres", 400);
...
async function requireAuth(request, env, requiredRole) {
  ...
  if ((LEVELS[user.funcao] || 0) < (LEVELS[requiredRole] || 0)) {
    throw httpError("Sem permissao", 403);
  }
...
function publicUser(user) {
  return {
    ...
    linha_padrao: user.linha_padrao,
    celula_padrao: user.celula_padrao,
